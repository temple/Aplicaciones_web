...
//   Carga del sistema de Autoloading
	...
	if (array_key_exists('app', $GLOBALS['config'])){
		...
		if (array_key_exists('db', $GLOBALS['config']['app'])){
			$DB_HOST = $GLOBALS['config']['app']['db']['host'];
			$DB_PORT = $GLOBALS['config']['app']['db']['port'];
			$DB_NAME = $GLOBALS['config']['app']['db']['scheme'];
			$DB_USER = $GLOBALS['config']['app']['db']['user'];
			$DB_PASS = $GLOBALS['config']['app']['db']['pass'];
			
			// Creamos una conexión a la base de datos
			$GLOBALS['db'] = new \PDO('mysql:host=' . $DB_HOST . ';port='.$DB_PORT.';dbname=' . $DB_NAME.";charset=utf8", $DB_USER, $DB_PASS);

			// Borramos la configuración
			unset($GLOBALS['config']['app']['db']);
			unset($DB_HOST);
			unset($DB_PORT);
			unset($DB_NAME);
			unset($DB_USER);
